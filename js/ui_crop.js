let _cropState = { scale: 1, x: 0, y: 0, imgW: 0, imgH: 0, onConfirm: null, minScale: 1 };
let _avatarCropState = { scale: 1, x: 0, y: 0, imgW: 0, imgH: 0, onConfirm: null, minScale: 1 };
...
function applyCropTransform() {
  ...
  const iw = _cropState.imgW * s;
  const ih = _cropState.imgH * s;
...
function openAvatarCropEditor(dataUrl, onConfirm) {
  ...
  img.onload = () => {
    ...
    const ch = container.offsetHeight;
    const minScale = Math.max(cw / img.naturalWidth, ch / img.naturalHeight);
    _avatarCropState.minScale = minScale;
...
function applyAvatarCropTransform() {
  ...
  const iw = _avatarCropState.imgW * s;
  const ih = _avatarCropState.imgH * s;
...
function confirmAvatarCrop() {
  ...
  const ch = container.offsetHeight;
  const s = _avatarCropState.scale;
  const sx = -_avatarCropState.x / s;
  const sy = -_avatarCropState.y / s;
  const sw = cw / s;
  const sh = ch / s;
  const SIZE = 600;
  const canvas = document.createElement('canvas');
...
function setupAvatarCropInteraction() {
  ...
  window._avatarMouseMove = e => {
    if (!dragging) return;
    _avatarCropState.x += e.clientX - lastMX;
    _avatarCropState.y += e.clientY - lastMY;
    lastMX = e.clientX;
    lastMY = e.clientY;
    applyAvatarCropTransform();
  };
  window._avatarMouseUp = () => {
    dragging = false;
...
function setupCropInteraction() {
  ...
  window.onmousemove = e => {
    if (!dragging) return;
    _cropState.x += e.clientX - lastMX;
    _cropState.y += e.clientY - lastMY;
    lastMX = e.clientX;
    lastMY = e.clientY;
    applyCropTransform();
  };
  ...
  container.ontouchmove = e => {
    ...
    if (e.touches.length === 1 && lastTouches?.length === 1) {
      ...
      const ox = midX - rect.left;
      const oy = midY - rect.top;
      const newScale = Math.min(3, Math.max(_cropState.minScale, _cropState.scale * ratio));
      const sr = newScale / _cropState.scale;
      _cropState.x = ox - (ox - _cropState.x) * sr;
      _cropState.y = oy - (oy - _cropState.y) * sr;
      _cropState.scale = newScale;
    }
    lastTouches = e.touches;
    applyCropTransform();
